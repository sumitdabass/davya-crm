#!/usr/bin/env python3
"""
Bulk-backfill all 3 lead source sheets into the CRM.

Push order: Sumit (lowest priority) -> Nikhil -> Sonam (highest priority)
so that any historical wrong-tier rows get auto-corrected via the
LeadIntakeService MERGE path.

Outcomes per row:
  201 created  -> new student (or merged-up from lower-tier existing)
  409 duplicate -> same/lower tier exists; silently skipped
  422 invalid  -> phone/course missing or malformed; CRM rejected
  5xx          -> server error (logged, retry-once)
"""
import csv
import json
import os
...
def map_sumit(row):
    ...
    return {
        ...
        "name":    (row[1] if len(row) > 1 else "").strip()[:120],
        "email":   (row[2] if len(row) > 2 else "").strip()[:120] or None,
        "state":   (row[5] if len(row) > 5 else "").strip()[:40] or None,
        "remarks": (row[6] if len(row) > 6 else "").strip()[:2000] or None,
        "source":  "Sumit Website",
        "owner_name": "Sumit",
    }
...
SHEETS = [
    ("sumit",  "/tmp/sheet_sumit.json",  map_sumit),
...
def post(payload):
    ...
    if code == 429:
        time.sleep(65)  # full rate-limit window
        code, body = post_once(payload)
    return code, body
...
def main():
    ...
    for label, path, mapper in SHEETS:
        ...
        rows = d.get("values", [])[1:]  # skip header
        ...
        for i, r in enumerate(rows, start=2):
            ...
            if not payload.get("phone") or len(payload["phone"]) != 10 or not payload.get("course"):
                ...
                continue
            if dry:
                if i <= 3 or i >= len(rows):  # preview a few
                    print(f"  [{i}] {payload['phone']:11} {payload['course'][:18]:18} -> {payload}")
                continue
            ...
            # incremental save every 50 rows so progress is preserved on crash
            ...
            time.sleep(1.1)  # 55 req/min — under Laravel's default 60/min throttle
        print(f"  TOTAL {label}: {per}")
